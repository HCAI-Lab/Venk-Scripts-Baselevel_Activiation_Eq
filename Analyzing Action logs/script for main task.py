...
def process_main_task_log(log_data):
    main_task_summary = {"MainTask": {}}
    
    previous_elapsed_time_ae = 0
    previous_completed_tasks = set()
    active_subtasks = {}  # Tracks active subtasks
    last_active_task = None  # Last active main task
    main_task_started = False  # Flag to track when main task begins

    for time_frame, data in log_data.items():
        # Detect the start of the main task (after END_ARRANGEMENT)
        if not main_task_started:
            if data.get("action") == "END_ARRANGEMENT" and data.get("result") == "SUCCESS":
                main_task_started = True
            continue  # Skip frames until main task starts

        # Detect the end of the main task
        if data.get("action") == "END_MAIN" and data.get("result") == "SUCCESS":
            break  # Stop processing after END_MAIN
        ...
        for task in new_tasks:
            if task in active_subtasks:
                ...
                task_key = f"Subtask_{len(main_task_summary['MainTask']) + 1}"
                ...
                main_task_summary["MainTask"][task_key] = task_data
...
# Process the log
main_task_summary = process_main_task_log(log_data)

# Save output JSON file
with open(r"swlee\main_task_summary.json", "w") as output_file:
    json.dump(main_task_summary, output_file, indent=4)

print("Main Task summary saved to main_task_summary.json")
