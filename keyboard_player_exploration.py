...
def keyboard_play(env, first_view_frames, is_rotate, rotate_per_frame):
    ...
    while True: 
        ...
        if keystroke == ord(actionList["ObjectState"]):
            """
        ...
        if keystroke == ord(actionList["MoveAhead"]):
            ...
            if event.metadata['lastActionSuccess']:
                ...
                log_and_print(f"> t={time_frame:03d}({elapsed_time:.2f}s)|SUCCESS| action = {action} & {agent_info}")
            else:
                log_and_print(f"> t={time_frame:03d}({elapsed_time:.2f}s)|<span style='color:red;'>FAIL</span>| action = {action}  <span style='color: red;'>&lt;!!!WARNING: CANNOT MOVE FORWARD FURTHER - OBSTACLE!!!&gt;</span>")
            time_frame += 1
