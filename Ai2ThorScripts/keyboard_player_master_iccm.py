...
import requests
from eval_user_test_iccm import GameStatus, MAIN_TASK, ARRANGEMENT_TASK, EXPLORATION_TASK
...
def log_and_print(message, update_index=False, clear_previous=False):
    print(message)  # Redirect output to Flask server
    
    try:
        if clear_previous:
            # Send a clear command before adding new logs
            requests.post('http://127.0.0.1:5001/update', data={'log': "CLEAR_LOGS"})
...
def get_interact_object(env, action, unique_id_map, pickup=None):
    ...
    if action in ['PickupObject', 'PutObject', 'OpenObject', 'CloseObject', 'ToggleObjectOn', 'ToggleObjectOff', 'SliceObject']:
        for index, (obj, _) in enumerate(candidates, start=1):
            ...
            text = f"[{index:02d}] " + unique_id_map[obj['objectId']] # Get the unique ID text
            text_size = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 1.0, 2)[0]
            text_x = x1 + (x2 - x1 - text_size[0]) // 2
            text_y = y1 + (y2 - y1 + text_size[1]) // 2
            frame = cv2.putText(frame, text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)  # Unique ID text
    ...
    log_and_print('<b>============================================================================</b>')
    log_and_print(f"<b>For '<span style='color:red;'>{action}</span>' - Type the corresponding two-digit index</b>")
    ...
    while True:
        ...
        if keystroke == 27:  # ESC key
            ...
            log_and_print('<b>============================================================================</b>')
            # log_and_print("UPDATE_INDEX::CLEAR", clear_previous=True)  # Clear input when ESC is pressed
...
def select_destination(env, locations, unique_id_map):
    ...
    for index, key in enumerate(locations.keys(), start=1):
        log_and_print(f"[{index:02d}] '<span style='color:blue;'>{key}</span>'" )
    ...
    while True:
        keystroke = cv2.waitKey(0)
        if keystroke == 27:  # ESC key
            log_and_print("UPDATE_INDEX::CLEAR", clear_previous=True)
        ...
        if keystroke in (8, 127):  # Backspace
            ...
            if 1 <= selected_index <= len(locations):
                ...
                log_and_print("UPDATE_INDEX::CLEAR", clear_previous=True)  # Clear the index display before confirmation
                ...
                log_and_print("UPDATE_INDEX::CLEAR", clear_previous=True)  # Clear previous input
                log_and_print("<b>!!! Invalid input. Please press a valid two-digit index !!!</b>")
                selected_index = ""
        else:
            continue
...
def keyboard_play(env, first_view_frames, is_rotate, rotate_per_frame, house):
    ## use keyboard control agent
    log_and_print(f"<b>=</b>" * 70)
    log_and_print(f"<b>! THE ARRANGMENT TASK BEGINS!</b>")
    log_and_print(f" -- Please press 'Q' to check object states (i.e., its capability or current states)")
    log_and_print(f" -- Please press 'Q' to check what objects exist in view at different locations.")
    ...
    log_and_print(f" -- You may not be able to interact with an object if it's being occluded by other objects.")
    log_and_print(f" -- You may not be able to interact with an object if it's located farther than reachable distance.")
    ...
    teleport_action = { # FloorPlan26
        "action": "TeleportFull",
        "position":{"x": -0.5, 'y':0.91, "z": 1.5},
        "rotation": {"x": 0, 'y':270, "z": 0},
        "horizon": 30 , # Set horizon level to default view
        'standing': True
    }

    event = env.step(teleport_action)
    cv2.imshow("first_view", cv2.cvtColor(event.frame, cv2.COLOR_RGB2BGR))

    start_time = time.time()  # Record the start time
    seen_objects = get_seen_objects(event, unique_id_map)
    reachable_distance = 2.0  # Define the maximum reachable distance

    log_and_print(f"> t={0:03d}({0.00}s)|BEGIN|", clear_previous=True)
    logs = {}
    logs[0] = log_json(event, 0, 0, None, "BEGIN", seen_objects)

    situation_board = set()
    exploration_task_completion = False
    arrangement_task_completion = False
    main_task_completion = False

    time_frame = 1
    while True: 
        keystroke = cv2.waitKey(0)
        log_and_print(f"", clear_previous=True)
        ...
        if keystroke == ord(actionList["GameStatus"]):
            ...
            game_status_data = {
                unique_id_map[obj['objectId']]: {
                    ...
                    'isOpen': obj['isOpen'],
                    'isFilledWithLiquid' : obj['isFilledWithLiquid'],
                    'assetId': obj['assetId'],
                }
            ...
            game_status_evaluator = GameStatus(game_status_data, agent_info, EXPLORATION_TASK, ARRANGEMENT_TASK, MAIN_TASK)

            ## Game status check for 'EXPLORATION_TASK' -------------------------------------------------------------------------------
            if exploration_task_completion == False and arrangement_task_completion == False and main_task_completion == False:
                all_completed, report = game_status_evaluator.evaluate_task_completion_exploration()
                ...
                if len(situation_board) == num_total_tasks:
                    log_and_print(f"<b>* All the exploration tasks were completed!</b>")
                    log_and_print("-" * 70)
                    log_and_print(f"<b>************************** END THE EXPLORATION TASK **************************</b>")

                    elapsed_time = time.time() - start_time  
                    seen_objects = get_seen_objects(event, unique_id_map)
                    logs[time_frame] = log_json(event, elapsed_time, elapsed_time, "END_EXPLORATION", "SUCCESS", seen_objects, None, None, completed_task=list(situation_board))

                    all_completed = True        
                
                    teleport_action = { 
                        "action": "TeleportFull",
                        "position":{"x": -0.5, 'y':0.91, "z": 1.5},  # Use the coordinates for teleportation when provided
                        "rotation": {"x": 0, 'y':270, "z": 0},
                        "horizon": 30 , 
                        'standing': True
                    }
                    ...
                    cv2.imshow("first_view", cv2.cvtColor(event.frame, cv2.COLOR_RGB2BGR))
                    situation_board.clear()
                    log_and_print(f"<b>=</b>" * 70)
                    log_and_print(f"<b>! THE MAIN HOUSEHOLD TASK BEGINS!</b>")

                    exploration_task_completion = True
                    time_frame += 1
            ...
            elapsed_time = time.time() - start_time  
            seen_objects = get_seen_objects(event, unique_id_map)
            logs[time_frame] = log_json(event, elapsed_time, elapsed_time, "GameStatusCheck", "SUCCESS", seen_objects, None, None, completed_task=list(situation_board))
            time_frame += 1
            continue


            ## Game status check for 'ARRANGEMENT_TASK' -------------------------------------------------------------------------------
            if exploration_task_completion == True and arrangement_task_completion == False and main_task_completion == False:
                all_completed, report = game_status_evaluator.evaluate_task_completion_arrangement()
                num_total_tasks = len(report)
                num_completed_tasks = sum(1 for details in report.values() if details['completed'])
                num_incomplete_tasks = num_total_tasks - num_completed_tasks
                ...
                log_and_print(f"<b>* [GAME STATUS CHECK] Current Game status = {len(situation_board)} out of {num_total_tasks} tasks were done!</b>")
                if len(situation_board) == num_total_tasks:
                    log_and_print(f"<b>* All the arragement tasks were completed!</b>")
                    log_and_print("-" * 70)
                    log_and_print(f"<b>************************** END THE ARRANGEMENT TASK **************************</b>")

                    held_obj_name = unique_id_map[held_object[0]["objectId"]] if (held_object := env.last_event.metadata.get("inventoryObjects", [])) else None
                    elapsed_time = time.time() - start_time  
                    seen_objects = get_seen_objects(event, unique_id_map)
                    logs[time_frame] = log_json(event, elapsed_time, elapsed_time, "END_ARRANGEMENT", "SUCCESS", seen_objects, None, None, held_obj_name, completed_task=list(situation_board))

                    all_completed = True

                    ## To make the user wait for 3 minutes.
                    frame_height, frame_width = 1600, 1600  # Adjust based on your window size
                    blank_frame = np.zeros((frame_height, frame_width, 3), dtype=np.uint8)
                    Countdown = 3 * 60  # 1 minute for testing (change to 5 minutes for actual usage)
                    with tqdm(total=Countdown, desc="Time left", unit="s") as pbar:
                        countdown_start = time.time()
                        while True:
                            blank_elapsed_time = time.time() - countdown_start
                            remaining_time = int(Countdown - blank_elapsed_time)
                            if remaining_time <= 0:
                                break

                            ## Update progress bar
                            pbar.n = Countdown - remaining_time
                            pbar.last_print_n = Countdown - remaining_time
                            pbar.refresh()
                            cv2.putText(blank_frame, "Please wait until the main household task begins.", 
                                        (frame_width // 7, frame_height // 4),
                                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
                            cv2.putText(blank_frame, "Please take your hands off from the keyboard.", 
                                        (frame_width // 7, frame_height // 3),
                                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
                            cv2.putText(blank_frame, "Begin the main task after this black image disappears.", 
                                        (frame_width // 7, frame_height // 2),
                                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
                            cv2.imshow("first_view", blank_frame) # Display the blank frame

                            time.sleep(1) # Sleep for a second to avoid excessive updates
                    ...
                    ## Teleport the agent to the desired position before restoring the window
                    teleport_action = { # FloorPlan26
                        "action": "TeleportFull",
                        "position":{"x": -0.5, 'y':0.91, "z": 1.5},
                        "rotation": {"x": 0, 'y':270, "z": 0},
                    ...
                    event = env.step(teleport_action)
                    ## Restore the window and show the new view after teleportation
                    cv2.imshow("first_view", cv2.cvtColor(event.frame, cv2.COLOR_RGB2BGR))
                    situation_board.clear()
                    log_and_print(f"<b>=</b>" * 70)
                    log_and_print(f"<b>! THE MAIN HOUSEHOLD TASK BEGINS!</b>")
                    log_and_print(f" -- 'ObjectStateCheck(Q)' does NOT provide the object information with name and bounding box.")
                    log_and_print(f" -- Please press 'Q' to check object states (i.e., its capability or current states)")
                    log_and_print(f" -- Please press 'G' to check the current game status (i.e., Task completion status)")
                    log_and_print(f" -- Please try a different location if 'PutObject' action fails.")
                    log_and_print(f" -- You may not be able to interact with an object if it's being occluded by other objects.")
                    ...
                    arrangement_task_completion = True
                    time_frame += 1
            ...
            ## Game status check for 'MAIN_TASK' -------------------------------------------------------------------------------
            if arrangement_task_completion == True and main_task_completion == False:
                all_completed, report = game_status_evaluator.evaluate_task_completion_main()
                num_total_tasks = len(report)
                num_completed_tasks = sum(1 for details in report.values() if details['completed'])
                num_incomplete_tasks = num_total_tasks - num_completed_tasks
                ...
                if len(situation_board) == num_total_tasks:
                    ...
                    log_and_print("-" * 70)
                    log_and_print(f"<b>************************** END THE GAME **************************</b>")

                    held_obj_name = unique_id_map[held_object[0]["objectId"]] if (held_object := env.last_event.metadata.get("inventoryObjects", [])) else None
                    elapsed_time = time.time() - start_time  
                    seen_objects = get_seen_objects(event, unique_id_map)
                    logs[time_frame] = log_json(event, elapsed_time, elapsed_time, "END_MAIN", "SUCCESS", seen_objects, None, None, held_obj_name, completed_task=list(situation_board))
        ...
        ## Check Object State (Modified) ############################################################################################################################
            ...
            ## Collect interactable objects with full metadata
            frame = env.last_event.frame.copy() 
            if arrangement_task_completion == False:
            ...
            ## Return a distinct value when no candidates are found
            if not candidates:
                log_and_print(f"> t={time_frame:03d}({elapsed_time:.2f}s)|<span style='color:red;'>FAIL</span>| action = CheckObjectState  <span style='color: red;'>&lt;! WARNING: No objects capable of state changes found within reachable distance in view !&gt;</span>", clear_previous=True)
                ...
                for index, (obj, distance) in enumerate(candidates, start=1):  # Start indices at 1
                    ...
                    if obj.get('canFillWithLiquid', False):
                        fill_status = "isFilledWithLiquid" if obj.get('isFilledWithLiquid') else "isNotFilledWithLiquid"
                        obj_details.append(f"{fill_status}")
                        obj_capability.append(f"canFillWithLiquid")
        ...
        ## FloorPlan26
        # LOCATIONS ={ 
        #     "Sink&Microwave":{ "action": "TeleportFull", "position":{"x": -0.5, 'y':0.91, "z": 1.25}, "rotation": {"x": 0, 'y':180, "z": 0}, "horizon": 30 ,'standing': True}
        #     , "CounterTop": {"action": "TeleportFull", "position":{"x": -2.5, 'y':0.91, "z": 1.25}, "rotation": {"x": 0, 'y':180, "z": 0}, "horizon": 30 ,'standing': True}
        #     , "CoffeeMachine":{ "action": "TeleportFull", "position":{"x": -2.0, 'y':0.91, "z": 3.0}, "rotation": {"x": 0, 'y':270, "z": 0}, "horizon": 30 ,'standing': True}
        #     , "Fridge": {"action": "TeleportFull", "position":{"x": -1.75, 'y':0.91, "z": 4.5}, "rotation": {"x": 0, 'y':270, "z": 0}, "horizon": 30 ,'standing': True}
        #     , "Stove": {"action": "TeleportFull", "position":{"x": -1.0, 'y':0.91, "z": 4.5}, "rotation": {"x": 0, 'y':90, "z": 0}, "horizon": 30 ,'standing': True}
        #     , "DiningTable": {"action": "TeleportFull", "position":{"x": -1.25, 'y':0.91, "z": 3.25}, "rotation": {"x": 0, 'y':90, "z": 0}, "horizon": 30 ,'standing': True}
        #     , "Door": {"action": "TeleportFull", "position":{"x": -0.5, 'y':0.91, "z": 1.5}, "rotation": {"x": 0, 'y':270, "z": 0}, "horizon": 30 ,'standing': True}
        # }

        LOCATIONS = mapping.LOCATIONS[house]
        ...
        if keystroke == ord(actionList["MoveTo"]):
            ...
            if destination == "CANCEL":
                ...
                log_and_print(f"> t={time_frame:03d}({elapsed_time:.2f}s-{elapsed_time2:.2f}s)|CANCEL| action = {action}", clear_previous=True)
            ...
            if event.metadata['lastActionSuccess']:
                ...
                log_and_print(f"> t={time_frame:03d}({elapsed_time:.2f}s-{elapsed_time2:.2f}s)|SUCCESS| action = {action} & {agent_info}", clear_previous=True)
        ...
        ## INTERACTION = PickupObject ########################################################################################################################
            ...
            if held_object:
                ...
                log_and_print(f"> t={time_frame:03d}({elapsed_time:.2f}s)|<span style='color:red;'>FAIL</span>| action = {action}  <span style='color: red;'>&lt;!!!WARNING: You are holding {held_obj_name}!!!&gt;</span>", clear_previous=True)
                logs[time_frame] = log_json(event, elapsed_time, elapsed_time, action, "FAIL", get_seen_objects(event, unique_id_map), None, None, held_obj_name, completed_task=list(situation_board))
                time_frame += 1
            ...
            if objectId == "NOT_FOUND":
                log_and_print(f"> t={time_frame:03d}({elapsed_time:.2f}s)|<span style='color:red;'>FAIL</span>| action = {action}  <span style='color: red;'>&lt;!!!WARNING: No valid interactable objects found within reachable distance in view!!!&gt;</span>", clear_previous=True)
                logs[time_frame] = log_json(event, elapsed_time, elapsed_time, action, "FAIL", get_seen_objects(event, unique_id_map), None , None, held_obj_name, completed_task=list(situation_board))
                time_frame += 1
            ...
            if event.metadata['lastActionSuccess']:
                log_and_print(f"> t={time_frame:03d}({elapsed_time:.2f}s-{elapsed_time2:.2f}s)|SUCCESS| action = {action} & object = {obj_name}", clear_previous=True)
        ...
        ## Interaction = PutObject ############################################################################################   
            ...
            if held_object:
                ...
                if objectId == "CANCEL":  # User pressed '0' to cancel the action
                    elapsed_time2 = time.time() - start_time
                    log_and_print(f"> t={time_frame:03d}({elapsed_time:.2f}s-{elapsed_time2:.2f}s)|CANCEL| action = PutObject", clear_previous=True)
                    logs[time_frame] = log_json(event, elapsed_time, elapsed_time2, action, "CANCEL", get_seen_objects(event, unique_id_map), candidates, None, held_obj_name, completed_task=list(situation_board))
                ...
                if objectId == "NOT_FOUND":
                    elapsed_time = time.time() - start_time
                    log_and_print(f"> t={time_frame:03d}({elapsed_time:.2f}s)|<span style='color:red;'>FAIL</span>| action = {action}  <span style='color: red;'>&lt;!!!WARNING: No valid interactable receptacles found within reachable distance in view!!!&gt;</span>", clear_previous=True)
                    logs[time_frame] = log_json(event, elapsed_time, elapsed_time, action, "FAIL", get_seen_objects(event, unique_id_map), None, None, held_obj_name, completed_task=list(situation_board))
                    time_frame += 1
                    continue
                ...
                elapsed_time2 = time.time() - start_time
                if event.metadata['lastActionSuccess']:
                    log_and_print(f"> t={time_frame:03d}({elapsed_time:.2f}s-{elapsed_time2:.2f}s)|SUCCESS| action = PutObject ({obj_name} -> {receptacle_name})", clear_previous=True)
                    ...
                    if event.metadata['lastActionSuccess']:
                        ...
                        log_and_print(f"> t={time_frame:03d}({elapsed_time:.2f}s-{elapsed_time2:.2f}s)|<span style='color:red;'>FAIL</span>| action = {action}  <span style='color: red;'>&lt;!!!WARNING: Action failed!!!&gt;</span>", clear_previous=True)
                ...
                objectId, candidates = get_interact_object(env, action, unique_id_map)
                log_and_print(f"> t={time_frame:03d}({elapsed_time:.2f}s)|<span style='color:red;'>FAIL</span>| action = PutObject  <!!!WARNING: No object is being held!!!>", clear_previous=True)
        ...
        ## Interaction = OpenObject ############################################################################################  
            ...
            ## Close any opened objects before anything is opened
            openable_objects = [obj for obj in event.metadata['objects'] if obj.get('openable', False) and obj.get('isOpen', False)]
            for obj in openable_objects:
                env.step(action="CloseObject", objectId=obj['objectId'], forceAction=True)
            ...
            elapsed_time2 = time.time() - start_time
            if event.metadata['lastActionSuccess']:
                log_and_print(f"> t={time_frame:03d}({elapsed_time:.2f}s-{elapsed_time2:.2f}s)|SUCCESS| action = OpenObject & object = {obj_name}", clear_previous=True)
                logs[time_frame] = log_json(event, elapsed_time, elapsed_time2, action, "SUCCESS", get_seen_objects(event, unique_id_map), candidates, obj_name, held_obj_name, completed_task=list(situation_board))
            else:
        ...
        ## Interaction = SliceObject ############################################################################################  
            ...
            if objectId == "NO_KNIFE":
                elapsed_time = time.time() - start_time
                log_and_print(f"> t={time_frame:03d}({elapsed_time:.2f}s)|<span style='color:red;'>FAIL</span>| action = {action}  <span style='color: red;'>&lt;!!!WARNING: You must hold a Knife to perform SliceObject!!!!&gt;</span>", clear_previous=True)
                logs[time_frame] = log_json(event, elapsed_time, elapsed_time, action, "FAIL", get_seen_objects(event, unique_id_map), candidates, None, held_obj_name, completed_task=list(situation_board))
                time_frame += 1
                continue  # Skip the rest of the loop if no object was found
...
def main(scene_name=None, gridSize=0.25, rotateStepDegrees=30, BEV=False, slope_degree=45
    ...
    ## collect frame
    first_view_frames = []
    logs, unique_id_map = keyboard_play(controller, first_view_frames, is_rotate, rotate_per_frame, house)

    if logs is not False:
        logs[99999] = unique_id_map
...
if __name__ == "__main__":
    main(scene_name="FloorPlan26", ## FloorPlan13
